...
const ImageActions = ({ Buttons, toggleFullScreen, isFullScreen }) => {
  return (
    <div className="absolute flex gap-3 w-auto z-50 right-2 top-2">
      ...
      <div className="relative group">
        ...
        <div
          className="absolute right-0 mt-1 min-w-max bg-white border border-gray-200 rounded-md shadow-lg opacity-0 group-hover:opacity-100 group-hover:translate-y-0 -translate-y-2 pointer-events-none group-hover:pointer-events-auto transition-all duration-300 flex gap-1 py-2 px-2 z-50"
        >
          {Buttons.map(({ Title, Icon, onClick }) => (
            <button
              key={Title}
              onClick={onClick}
              className="flex items-center gap-2 text-gray-500 hover:text-blue-600 hover:bg-gray-100 rounded p-1 transition-all duration-200 scale-100 hover:scale-110"
              title={Title}
            >
              <Icon size={16} />
            </button>
